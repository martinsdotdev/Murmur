...
public sealed partial class MainWindow : Window
{
    ...
    private void PlayPauseAccelerator_Invoked(
    ...
    private async void ResetAccelerator_Invoked(
        KeyboardAccelerator sender,
        KeyboardAcceleratorInvokedEventArgs args
    )
    {
    ...
    private void GridViewAccelerator_Invoked(
        KeyboardAccelerator sender,
        KeyboardAcceleratorInvokedEventArgs args
    )
    {
    ...
    private void OnWindowClosed(object sender, WindowEventArgs args)
    {
        ...
        try
        {
            App.State?.SaveAsync().GetAwaiter().GetResult();
        }
        catch (Exception ex)
        {
            DiagnosticLog.Log($"Close save error: {ex.Message}");
        }
        ...
        {
            DiagnosticLog.Log($"Close mixer dispose error: {ex.Message}");
        }
    ...
    private async void QuitApp()
    {
        ...
        {
            DiagnosticLog.Log($"Quit save error: {ex.Message}");
        }
        ...
        if (App.Mixer is IAsyncDisposable mixer)
        {
            ...
            {
                DiagnosticLog.Log($"Quit mixer dispose error: {ex.Message}");
            }
        }
    ...
    private async void ResetMix_Click(object sender, RoutedEventArgs e) =>
        await RequestResetMixAsync();
    ...
    {
        if (ViewModel.ActiveSoundCount == 0)
            return;
        var ok = await ConfirmAsync(
            "Reset mix?",
    ...
    internal async Task RequestImportAsync()
    {
        ...
        if (sound is not null)
        {
            ViewModel.StatusMessage =
                $"Imported \"{sound.DisplayName}\". Drag its slider above 0 to mix it in.";
        }
    }
    ...
    internal async Task<string?> PromptForUrlAsync(string title)
    {
        ...
        {
            DiagnosticLog.Log($"Clipboard prefill skipped: {ex.Message}");
        }
