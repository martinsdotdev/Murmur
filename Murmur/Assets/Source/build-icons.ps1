...
# Padding for the non-square tiles, and the unplated glyph on a dark taskbar.
$BgColor   = '#FAF6EE'
# The unplated glyph on a light taskbar.
$FgColor   = '#21120E'
...
function Invoke-Magick {
    ...
    # Without this every PNG carries a creation date, so two renders of the same
    # SVG differ and -Verify can never pass. Leading position applies it to the
    # write regardless of what the caller passes after.
...
function New-MonoMaster {
    ...
    try {
        ...
        # Mono SVG is 432 units square. 432 * 480 / 72 = 2880, so the first raster
        # already exceeds the master size and the resize below only ever downscales.
        Invoke-Magick @('-background', 'none', '-density', '480', $tempSvg,
