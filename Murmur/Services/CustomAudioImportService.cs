...
public sealed class CustomAudioImportService
{
    ...
    public async Task<Sound?> ImportAsync(Window owner)
    {
        ...
        {
            DiagnosticLog.Log(
                $"Custom import failed during decode of '{storageFile.Path}': {ex.Message}"
            );
            try
            {
    ...
    {
        if (sound.Kind != SoundKind.CustomOgg)
            return;
        ...
        if (_state.CustomAudios.TryGetValue(sound.Id, out var path))
        {
            ...
            {
                DiagnosticLog.Log($"Custom audio file delete error: {ex.Message}");
            }
