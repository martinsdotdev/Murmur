...
{
    /// <summary>Not arbitrary: this is the UUID Blanket uses for its own default preset,
    /// so a user's settings could theoretically migrate between the two.</summary>
    ...
    [ObservableProperty]
    public partial double MasterVolume { get; set; } = 0.7;
    ...
    [ObservableProperty]
    public partial string ActivePresetId { get; set; } = DefaultPresetId;
    ...
    private Task<string> SerializeOnUIThreadAsync()
    {
        ...
        if (
            !_dispatcher.TryEnqueue(() =>
            {
                try
                {
                    tcs.SetResult(JsonSerializer.Serialize(this, s_jsonOptions));
                }
                catch (Exception ex)
                {
                    tcs.SetException(ex);
    ...
    private static Preset MakeDefaultPreset() =>
        new()
        {
            Id = DefaultPresetId,
            VisibleName = "Default",
