...
public sealed class AudioGraphMixerService : IMixerService
{
    ...
    public double MasterVolume
    {
        ...
        set
        {
            ...
            if (_output is not null && IsPlaying)
    ...
    private async Task InitializeAsyncCore()
    {
        ...
        _graph.UnrecoverableErrorOccurred += (_, args) =>
            DiagnosticLog.Log($"!! Graph error: {args.Error}");
        _graph.Start();
    ...
    {
        Pause();
        foreach (var entry in _sounds.Values)
            DetachInputNode(entry);
        _sounds.Clear();
        foreach (var entry in _streamingSounds.Values)
            DetachStreamingInputNode(entry);
    ...
    public Task RegisterStreamingSoundAsync(Sound sound, Func<Task<MediaSource>> resolveMediaSource)
    {
        ...
        if (!_streamingSounds.TryAdd(sound.Id, entry))
            throw new InvalidOperationException(
                $"Streaming sound '{sound.Id}' is already registered."
            );
    ...
    public void SetSoundVolume(string soundId, double volume)
    {
        ...
        if (_sounds.TryGetValue(soundId, out var entry))
        {
            ...
            if (shouldBeActive && entry.InputNode is null)
                AttachInputNode(entry);
            else if (!shouldBeActive && entry.InputNode is not null)
                DetachInputNode(entry);
            else if (entry.InputNode is not null)
                entry.InputNode.OutgoingGain = volume;
            return;
        }
        ...
        if (_streamingSounds.TryGetValue(soundId, out var sEntry))
        {
            ...
            if (shouldBeActive && sEntry.InputNode is null)
                _ = AttachStreamingInputNodeAsync(sEntry);
            else if (!shouldBeActive && sEntry.InputNode is not null)
                DetachStreamingInputNode(sEntry);
            else if (sEntry.InputNode is not null)
                sEntry.InputNode.OutgoingGain = volume;
            return;
        }
    ...
    {
        if (_sounds.TryGetValue(soundId, out var e))
            return e.Volume;
        if (_streamingSounds.TryGetValue(soundId, out var sE))
            return sE.Volume;
        return 0.0;
    }
    ...
    private async Task AttachStreamingInputNodeAsync(StreamingSoundEntry entry)
    {
        ...
        if (Interlocked.CompareExchange(ref entry.AttachInFlight, 1, 0) != 0)
            return;
        ...
        try
        {
            ...
            if (
                !_streamingSounds.ContainsKey(entry.Sound.Id)
            ...
            {
                DiagnosticLog.Log(
                    $"Streaming node create threw for '{entry.Sound.Id}': {ex.Message}"
                );
                StreamAttachFailed?.Invoke(
                    this,
            ...
            {
                DiagnosticLog.Log(
                    $"Streaming node create failed for '{entry.Sound.Id}': {result.Status} ({result.ExtendedError?.Message})"
                );
                StreamAttachFailed?.Invoke(
                    this,
    ...
    )
    {
        ...
        float[] pcm = entry.Pcm;
        if (requiredSamples <= 0 || pcm.Length == 0)
            return;
