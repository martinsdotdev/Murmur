...
public sealed class PresetService
{
    ...
    public Preset SaveCurrentAs(string visibleName)
    {
        ...
        foreach (var sound in _catalog.All)
        {
            ...
            if (v > 0)
                preset.SoundVolumes[sound.Id] = v;
        }
    ...
    {
        if (presetId == AppState.DefaultPresetId)
            return;
    ...
    {
        if (_state.ActivePreset is not { } preset)
            return;
        if (volume > 0)
            preset.SoundVolumes[soundId] = volume;
        else
    ...
    private int IndexOfActive()
    {
        for (int i = 0; i < _state.Presets.Count; i++)
            if (_state.Presets[i].Id == _state.ActivePresetId)
                return i;
        return 0;
    }
}
