...
public sealed class YouTubeAudioImportService
{
    ...
    private async Task<MediaSource> ResolveMediaSourceAsync(string watchUrl)
    {
        ...
        if (video.Duration is null)
        {
            ...
            {
                DiagnosticLog.Log(
                    $"Live resolve failed for '{video.Id}' (Duration null), falling back to manifest. {ex.GetType().Name}: {ex.Message}"
                );
        ...
        {
            DiagnosticLog.Log(
                $"Manifest resolve failed for '{video.Id}' (Duration {video.Duration}), falling back to live. {ex.GetType().Name}: {ex.Message}"
            );
            return await CreateLiveMediaSourceAsync(video);
        }
    }
    ...
    public static bool IsAcceptableYouTubeUrl(string watchUrl, out string canonical)
    {
        ...
        if (string.IsNullOrWhiteSpace(watchUrl))
            return false;
        if (!Uri.TryCreate(watchUrl.Trim(), UriKind.Absolute, out var parsed))
            return false;
        if (parsed.Scheme != Uri.UriSchemeHttps)
            return false;
        if (
            parsed.Host
            is not "www.youtube.com"
                and not "youtube.com"
                and not "youtu.be"
                and not "m.youtube.com"
        )
            return false;
    ...
    public async Task RestoreSavedAsync()
    {
        ...
        foreach (var (id, record) in entries)
        {
            try
            {
                ...
                await _mixer.RegisterStreamingSoundAsync(
                    sound,
            ...
            {
                DiagnosticLog.Log(
                    $"YouTube restore failed for '{id}' ({record.WatchUrl}): {ex.Message}"
                );
            }
        }
    }
