...
public sealed partial class SoundTileControl : UserControl
{
    public static readonly DependencyProperty ViewModelProperty = DependencyProperty.Register(
        nameof(ViewModel),
        typeof(SoundCardViewModel),
        typeof(SoundTileControl),
    ...
    // should fire even when a tile has focus, matching media-player conventions.
    ...
    {
        var control = (SoundTileControl)d;
    ...
    {
        if (
            e.PropertyName == nameof(SoundCardViewModel.IsActive)
            && sender is SoundCardViewModel vm
        )
        {
            _isActive = vm.IsActive;
            UpdateState();
    ...
    private bool IsWithinSlider(DependencyObject? element)
    {
        ...
        {
            if (ReferenceEquals(element, VolumeSlider))
                return true;
